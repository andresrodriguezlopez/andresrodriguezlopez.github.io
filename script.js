...
function renderMediaItem(item, title){
  ...
  if (isVideo && isLocalVideo(item.src)){
    ...
    lbViewer.innerHTML = `<iframe src="${withAutoplay(item.src)}" title="${title}" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>`;
  } else {
    lbViewer.innerHTML = `
      <img src="${item.src}" alt="${title}">
      <button class="lightbox-fullscreen-btn" aria-label="View fullscreen" title="View fullscreen">
        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          <path d="M8 3H5a2 2 0 0 0-2 2v3"/><path d="M21 8V5a2 2 0 0 0-2-2h-3"/><path d="M3 16v3a2 2 0 0 0 2 2h3"/><path d="M16 21h3a2 2 0 0 0 2-2v-3"/>
        </svg>
      </button>
    `;
    const fsBtn = lbViewer.querySelector('.lightbox-fullscreen-btn');
    const imgEl = lbViewer.querySelector('img');
